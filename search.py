...
def DFSearch(dict, start, end):
  ...
  route = route.replace(']' , '')
  route = route.replace(',' , ' ->')
  return 'The route from ' + str(start.upper()) + ' to ' + str(end.upper()) + ' is: ' + route
    ...
    while found==False:
      for i in dict[station]: #Expanding node
        if i.upper() == end:
          ...
          cameFromAux.append(station)
      #Here we have the schedule loaded
      expanded.append(schedule[0]) #Add to expanded the first added in schedule
      cameFrom.append(cameFromAux[0])
      schedule.pop(0) #Delete the first one in schedule
      cameFromAux.pop(0) #Delete the last one in cameFrom
      station = expanded[-1]
